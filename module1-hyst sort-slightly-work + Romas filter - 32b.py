...
def find_n_circle(point_cloud):
    ...
    for one_point in point_cloud.points:
        ...
        scanID = 0
        scanID = int((angle + (n_beams * 2 - 1)) / 2 + 0.5)
        print(scanID)
...
def is_on_radius(edge_points_array):
    ...
    for e in comb:
        ...
        print(P)
        #print(R)
        circule_center_points_array.append(np.array(P))
    return P
...
##
   ...
   #draw_geometries([dummy_cloud])
    ...
    points_su_left.append(is_on_radius(edge_point_array_left))
    print(points_su_left)
    points_su_right.append(is_on_radius(edge_point_array_right))
